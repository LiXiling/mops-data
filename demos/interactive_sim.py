...
def solve(env: BaseEnv, debug=False, vis=False):
    ...
    if env.unwrapped.robot_uids == "panda_stick":
        ...
        env.unwrapped.robot_uids == "panda"
        or env.unwrapped.robot_uids == "panda_wristcam"
    ):
    ...
    def select_panda_hand():
        viewer.select_entity(
            sapien_utils.get_obj_by_name(env.agent.robot.links, "panda_hand")
            ._objs[0]
            .entity
        )
        ...
        if execute_current_pose:
            ...
            if (
                ...
                result = planner.move_to_pose_with_screw(
                    transform_window._gizmo_pose * sapien.Pose([0, 0, 0.1]),
                    dry_run=True,
                )
            elif env.unwrapped.robot_uids == "panda_stick":
                result = planner.move_to_pose_with_screw(
                    transform_window._gizmo_pose * sapien.Pose([0, 0, 0.15]),
                    dry_run=True,
                )
