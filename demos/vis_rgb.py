...
def vis_rgb(h5_path: str, n: int = 4, seed: int = 0) -> None:
    with h5py.File(h5_path, "r") as f:
        ...
        selected_train = rng.choice(train_keys, size=min(n, len(train_keys)), replace=False)
        selected_test  = rng.choice(test_keys,  size=min(n, len(test_keys)),  replace=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualize train/test RGB images from an HDF5 dataset.")
    parser.add_argument("h5_path", nargs="?", default="data/mops_data/mops_kitchen_dataset.h5",
